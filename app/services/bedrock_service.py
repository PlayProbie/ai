...
from app.core.prompts import QUESTION_GENERATION_SYSTEM_PROMPT, QUESTION_FEEDBACK_SYSTEM_PROMPT
...
class BedrockService:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.error(f"❌ Bedrock 모델 초기화 실패: {type(error).__name__}: {error}")
            raise AIModelNotAvailableException(f"Bedrock 모델 초기화 실패: {error}")
    ...
    def invoke(self, prompt: str) -> str:
        ...
        try: 
            ...
            raise AIGenerationException(f"Bedrock API 호출 실패: {error}")

    def generate_fixed_questions(self, request: FixedQuestionDraftCreate) -> FixedQuestionDraft:
        """
        ...
        try:
            ...
            raise AIGenerationException(f"질문 생성 중 오류 발생: {error}")

    def generate_feedback_questions(self, request: FixedQuestionFeedbackCreate) -> FixedQuestionFeedback:
        """
        ...
        try:
            ...
            # 호출 시 LLM 생성
            structured_llm = self.chat_model.with_structured_output(FixedQuestionFeedback)
            chain = prompt | structured_llm
            ...
            raise AIGenerationException(f"질문 피드백 생성 중 오류 발생: {error}")
