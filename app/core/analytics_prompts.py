...
SURVEY_SUMMARY_PROMPT = """## 질문별 분석 요약
{question_summaries}

## 작업
위 질문별 분석 요약들을 종합하여, 이 설문 전체에서 드러나는 핵심 인사이트를 1~2문장으로 작성하세요.
테스터들의 전반적인 게임 경험과 개선이 필요한 부분을 간결하게 정리하세요.


### 출력 형식 (JSON)
- 장점과 단점을 모두 포함하여 자연스럽게 작성하세요.
- 반드시 "~입니다", "~습니다" 등 존댓말(합니다체)로 작성하세요.
{{
  "survey_summary": "설문 전체를 종합한 핵심 평가 1~2문장"
}}
