...
from app.schemas.survey import EndReason

GAME_ELEMENT_EXTRACTION_PROMPT = """다음 게임 정보를 분석하여 핵심 요소를 추출해주세요.
...
1. VALID: 질문 주제와 의미적 연관성이 있는 응답
   ...
   - "없다", "없었다", "딱히 없다"는 실제 의견이므로 VALID
   - REFUSAL은 "패스", "다음 질문" 같은 명확한 거부 의사만 해당
   - "개선점이 없다", "불편한 점이 없었다" 등은 유효한 피드백임

2. OFF_TOPIC: 질문과 거의 관계가 없더라도, 대화 흐름상 조금이라도 용인될 수 있다면 VALID입니다.
   - **절대로 OFF_TOPIC을 남발하지 마세요.**
   - **다음의 경우에만 OFF_TOPIC으로 분류하세요:**
     - 명백한 스팸, 광고, 도배
     - 완전히 무의미한 문자열 나열 (예: "ㅋㅋㅋ", "asdf")
     - 게임이나 테스트와 전혀 0.1%도 상관없는 뚱딴지 소리 (예: "오늘 점심 뭐 먹지?", "배고파")
   - **다음은 모두 VALID입니다:**
     - 질문과 조금이라도 연관된 답변
     - 사용자의 감정 표현, 푸념, 농담 ("아 너무 어렵다", "이게 뭐냐")
     - "맵이 안 보여", "조작이 이상해" 등 구체적 불편함 호소
     - 심지어 다소 엉뚱해 보여도 비유적이거나 문맥상 이해 가능하면 무조건 VALID
...
5. REFUSAL: 명시적인 답변 거부 또는 회피
   - "패스할게요", "다음 질문으로", "답변하기 싫어요", "귀찮아요", "스킵"
...
# 판단 원칙
- **OFF_TOPIC 판정은 매우 신중해야 합니다. 100% 확신이 없다면 VALID로 분류하세요.**
...
## Thickness (맥락적 상세함)
응답에 구체적인 상황/맥락이 하나라도 포함되어 있으면 HIGH입니다.

체크리스트 (하나라도 해당하면 HIGH):
□ 구체적 게임 요소 언급 (캐릭터, 씬, 스테이지, 아이템, 스킬 등)
□ 상황 묘사 (무엇이 일어났는지, 어떤 장면이었는지)
□ 행동이나 과정 설명
...
## Richness (개념적 깊이)
응답에 감정, 느낌, 이유가 하나라도 포함되어 있으면 HIGH입니다.

체크리스트 (하나라도 해당하면 HIGH):
□ 감정/느낌 표현 (재밌었다, 무서웠다, 좋았다, 스릴있다 등)
□ 이유 설명 (~해서, ~때문에, ~라서)
□ 평가나 의견 (괜찮았다, 별로였다, 인상적이었다 등)
...
# 판정 원칙
- **관대하게 평가하세요!** 애매하면 HIGH로 판정하세요.
- 테스터가 성의있게 답변했다면 EMPTY가 되어선 안 됩니다.
- "재밌었다", "좋았다" 같은 간단한 감정 표현도 Richness HIGH입니다.
- "곰돌이가 뛰쳐왔다" 같은 구체적 언급도 Thickness HIGH입니다.
...
대화 맥락을 반드시 고려하여 자연스럽게 테스터의 답변에서 구체적인 상황과 맥락을 이끌어내는 꼬리질문을 생성해주세요.

# ⚠️ 핵심 규칙 (반드시 준수!)
**꼬리질문은 반드시 원본 질문과 동일한 주제여야 합니다!**
- 원본 질문이 "어려움/불편함"을 묻는다면 → 꼬리질문도 "어려움/불편함"에 대해 물어야 함
- 원본 질문이 "재미있었던 순간"을 묻는다면 → 꼬리질문도 "재미"에 대해 물어야 함
- 원본 질문이 "개선점"을 묻는다면 → 꼬리질문도 "개선점"에 대해 물어야 함
...
# 예시 (원본 질문에 맞춤)
- 원본이 "어려움"을 묻는 경우 → "혹시 그런 어려움을 느끼셨던 특정 장면이 있으셨나요?"
- 원본이 "재미"를 묻는 경우 → "혹시 그런 재미를 느끼셨던 특정 장면이 있으셨나요?"
- 원본이 "개선점"을 묻는 경우 → "구체적으로 어떤 부분이 개선되면 좋을지 예를 들어주실 수 있나요?"
...
대화 맥락을 반드시 고려하여 자연스럽게 테스터의 답변에서 이유와 원인을 탐색하는 꼬리질문을 생성해주세요.

# ⚠️ 핵심 규칙 (반드시 준수!)
**꼬리질문은 반드시 원본 질문과 동일한 주제여야 합니다!**
- 원본 질문 주제가 "어려움"이면 → "왜 어려우셨나요?"
- 원본 질문 주제가 "재미"이면 → "왜 재미있으셨나요?"
- 원본 질문 주제가 "개선점"이면 → "왜 개선이 필요하다고 생각하셨나요?"
...
대화 맥락을 반드시 고려하여 자연스럽게 테스터의 개인적인 경험과 감정을 탐색하는 꼬리질문을 생성해주세요.
...
# =============================================================================
# Session Service Prompts (Phase 2, 4.5, 5)
# =============================================================================

GREETING_PROMPT = """당신은 게임 플레이테스트 인터뷰어입니다.
이제 막 해당 게임을 플레이하고 온 테스터를 대상으로 인터뷰를 진행해야합니다.
...
# 예시
"{game_name}을 플레이해주셔서 감사합니다! 🎮 오늘은 {target_theme}에 대한 의견을 들어보려 합니다. 편하게 이야기해 주세요! 👋"
...
# =============================================================================
# Issue #7: RAG 기반 질문 생성
# =============================================================================

QUESTION_RAG_PROMPT = """당신은 게임 QA 및 FGT 전문가입니다.
주어진 **참고 질문(Reference Questions)** 의 스타일과 구조를 학습하여, 현재 게임에 최적화된 새로운 질문들을 생성해주세요.

# 게임 정보
- 이름: {game_name}
- 설명: {game_description}
- 핵심 요소: {extracted_elements}

# 참고 질문 (스타일 참고용)
아래 질문들은 우리 DB에서 검색된, 이 게임과 유사한 맥락의 우수한 질문들입니다.
이 질문들의 **의도, 깊이, 구체성**을 참고하되, 반드시 **현재 게임의 구체적인 내용**으로 바꿔서 질문하세요.
{reference_questions}

# 요청 사항
1. 위 참고 질문들의 형식을 빌려오되, 내용은 **{game_name}** 에 맞춰 완전히 새로 작성하세요.
2. 게임 설명에 언급된 고유 명사(캐릭터 이름, 스킬명, 지역명 등)를 적극적으로 활용하세요.
3. 질문 개수: 정확히 **{count}개**
4. 각 질문은 1문장으로 간결하게 작성하세요.
5. 한국어로, 정중한 존댓말을 사용하세요.
