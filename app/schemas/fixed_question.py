...
class FixedQuestionDraftCreate(BaseModel):
    ...
    game_genre: str = Field(..., description="게임 장르 (Enum Value: shooter, rpg, etc.)")
    game_context: str = Field(..., description="게임 상세 정보 및 배경 설정 (500자+)")
    test_purpose: str = Field(..., description="테스트 목적 (Enum Value: gameplay-validation, etc.)")
...
class FixedQuestionDraft(BaseModel):
    ...
    """
    questions: List[str] = Field(..., description="생성된 추천 질문 리스트 (Fixed Questions)")
...
class FixedQuestionFeedback(BaseModel):
    ...
    """
    candidates: List[str] = Field(..., min_length=3, max_length=3, description="수정된 추천 대안 질문 3개")
